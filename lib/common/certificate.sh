...
#
# Ported from the mature xrayctl certificate lifecycle and adapted so the
# certificate layer is engine-neutral. Certbot runs in an isolated ProxyCTL
# venv/config/work/log environment and never touches the system Certbot state.
# ------------------------------------------------------------------------------
...
cert_identifier_for_subject() {
    local subject="${1:-}"
    cert_validate_subject "$subject" || {
        error "Invalid certificate subject: ${subject}"
        return 1
    }
...
_cert_setup_runtime_access() {
    ...
    if ! getent group "$group" >/dev/null 2>&1; then
        command -v groupadd >/dev/null 2>&1 || {
            error 'groupadd is required to prepare certificate access.'
            return 1
        }
        groupadd --system "$group" || return 1
    fi

    if [[ -L "$PROXYCTL_CERTS" ]]; then
        error "Refusing symlink certificate root: ${PROXYCTL_CERTS}"
        return 1
    fi
    ...
    # systemd services get the shared certificate group without changing the
    # service's primary user. The drop-in takes effect on the next restart.
    if [[ "$(system_init 2>/dev/null || true)" == systemd ]]; then
        local engine service dropin changed=0
...
_cert_prepare_directory() {
    ...
    if [[ -e "$dir" && ! -d "$dir" ]]; then
        error "Certificate path is not a directory: ${dir}"
        return 1
    fi
...
cert_validate_pair_files() {
    ...
    command -v openssl >/dev/null 2>&1 || {
        error 'openssl is required for certificate validation.'
        return 1
    }
...
# Atomic pair replacement inherited from xrayctl: compare first, stage .new,
# validate pair, save .old, replace both, and restore the old state if either
# rename fails.
_cert_replace_pair() {
    ...
    cert_validate_pair_files "$source_cert" "$source_key" || {
        error 'Certificate or private key is invalid or mismatched.'
        return 1
    }
    _cert_prepare_directory "$id" || return 1

    dir=$(cert_dir "$id")
    cert_target="${dir}/fullchain.pem"
    key_target="${dir}/privkey.pem"
    group=$(cert_runtime_group)

    [[ ! -L "$cert_target" && ! -L "$key_target" ]] || {
        error "Refusing symlink certificate files in ${dir}"
        return 1
    }

    if ! cmp -s "$source_cert" "$cert_target" 2>/dev/null || ! cmp -s "$source_key" "$key_target" 2>/dev/null; then
        changed=1
    fi
    if (( changed == 0 )); then
    ...
    cert_bak="${dir}/.fullchain.old"
    key_bak="${dir}/.privkey.old"

    if ! install -m 640 -o root -g "$group" "$source_cert" "$cert_tmp"; then
        rm -f -- "$cert_tmp" "$key_tmp"
        return 1
    fi
    if ! install -m 640 -o root -g "$group" "$source_key" "$key_tmp"; then
        rm -f -- "$cert_tmp" "$key_tmp"
        return 1
    fi
    if ! cert_validate_pair_files "$cert_tmp" "$key_tmp"; then
        rm -f -- "$cert_tmp" "$key_tmp"
        error 'Staged certificate pair failed validation.'
        return 1
    fi
    ...
    if [[ -f "$cert_target" ]]; then
        had_cert=1
        cp -a -- "$cert_target" "$cert_bak" || { rm -f -- "$cert_tmp" "$key_tmp"; return 1; }
    fi
    if [[ -f "$key_target" ]]; then
        had_key=1
        cp -a -- "$key_target" "$key_bak" || {
    ...
    if (( need_rollback == 1 )); then
        rm -f -- "$cert_tmp" "$key_tmp"
        if (( had_cert == 1 )); then mv -f -- "$cert_bak" "$cert_target" || true; else rm -f -- "$cert_target"; fi
        if (( had_key == 1 )); then mv -f -- "$key_bak" "$key_target" || true; else rm -f -- "$key_target"; fi
...
# Best-effort engine-neutral consumer detection. It deliberately searches for
# the exact managed certificate paths instead of knowing Xray/sing-box TLS JSON.
cert_consumers() {
...
cert_ensure_certbot_environment() {
    ...
    if [[ ! -x "$bin" ]]; then
        need_install=1
    elif ! "$bin" --help all 2>/dev/null | grep -q -- '--ip-address'; then
        need_install=1
    elif ! "${venv}/bin/python" -c 'import certbot_dns_cloudflare' >/dev/null 2>&1; then
        need_install=1
    elif ! "${venv}/bin/python" -c 'import certbot_nginx' >/dev/null 2>&1; then
        need_install=1
    fi
    ...
    if (( need_install == 1 )); then
        ...
        if [[ ! -x "${venv}/bin/python" ]]; then
            if ! "$python" -m venv "$venv"; then
                ...
                "$python" -m venv "$venv" || {
                    error 'Unable to create the isolated Certbot virtual environment.'
                    return 1
                }
            fi
        fi
        ...
        if [[ ! -x "${venv}/bin/pip" ]]; then
            "${venv}/bin/python" -m ensurepip --upgrade >/dev/null 2>&1 || {
                error 'Unable to bootstrap pip in the Certbot virtual environment.'
                return 1
            }
        fi
        ...
        "${venv}/bin/pip" install --disable-pip-version-check --timeout 15 --retries 2 \
            --upgrade 'certbot>=5.4' certbot-dns-cloudflare certbot-nginx || {
            error 'Certbot or certificate plugin installation failed.'
            return 1
        }

        "$bin" --help all 2>/dev/null | grep -q -- '--ip-address' || {
            error 'Installed Certbot does not support IP certificates.'
            return 1
        }
        "${venv}/bin/python" -c 'import certbot_dns_cloudflare, certbot_nginx' >/dev/null 2>&1 || {
            error 'Certbot plugins failed their import check.'
            return 1
        }
    fi
...
_cert_save_cloudflare_credentials_locked() {
    ...
    printf '%s\n' \
        ...
        "dns_cloudflare_api_key = ${api_key}" >"$tmp" || { rm -f -- "$tmp"; return 1; }
    chmod 600 "$tmp" || { rm -f -- "$tmp"; return 1; }
    mv -f -- "$tmp" "$file"
}

cert_save_cloudflare_credentials() {
    local email="${1:-}" api_key="${2:-}"
    with_lock cert _cert_save_cloudflare_credentials_locked "$email" "$api_key"
}

cert_delete_cloudflare_credentials() {
    with_lock cert _cert_delete_cloudflare_credentials_locked
}
...
_cert_run_http_with_engine() {
    local engine="$1"
    shift
    local was_active=0 rc=0 restore_rc=0
    ...
    if (( was_active == 1 )); then
        engine_call "$engine" start || restore_rc=$?
    fi
...
_cert_issue_domain_cloudflare() {
    local domain="$1" email="$2" force="$3" file
    file=$(cert_cloudflare_file)
    cert_cloudflare_credentials_available || {
        error 'Cloudflare credentials are not configured.'
        return 1
    }
    local args=(certonly --dns-cloudflare --dns-cloudflare-credentials "$file" \
...
_cert_issue_domain_http() {
    local domain="$1" email="$2" force="$3" owner
    ...
    case "$owner" in
        ...
        nginx)
            ...
            ;;
        apache)
            error 'Port 80 is owned by Apache; automatic Apache validation is intentionally unsupported. Use Cloudflare DNS or manual DNS.'
            return 1
            ;;
        unknown|other)
            error 'Port 80 is occupied by an unknown process; refusing to stop it. Use DNS validation.'
            return 1
            ;;
    esac
}
...
_cert_issue_manual_dns() {
    ...
    local args=(certonly --manual --agree-tos -m "$email" --preferred-challenges dns --cert-name "$domain" -d "$domain")
    (( force == 0 )) || args+=(--force-renewal)
...
_cert_issue_ip() {
    local ip="$1" email="$2" identifier="$3" force="$4" owner
    owner=$(cert_detect_port80_owner)
    local args=(certonly --standalone --non-interactive --agree-tos \
...
_cert_sync_lineage() {
    local identifier="$1" cert_name="$2" __changed_var="${3:-}"
...
cert_setup_renewal_timer() {
    ...
    [[ "$(system_init 2>/dev/null || true)" == systemd ]] || {
        warn 'Automatic certificate timer is currently available only on systemd; use proxyctl cert renew-auto manually on this system.'
        return 0
    }
    [[ -x "$PROXYCTL_BIN" ]] || {
...
_cert_acme_issue_locked() {
    ...
    if ! network_validate_ip "$subject" 2>/dev/null; then
        cert_name=$(printf '%s' "$subject" | tr '[:upper:]' '[:lower:]')
    else
        method=ip
    fi
    ...
    case "$method" in
        ...
        http)
            ...
            if [[ "$(cert_detect_port80_owner)" == nginx ]]; then validation=http-nginx; fi
    ...
    if ! _cert_sync_lineage "$identifier" "$cert_name" changed; then
        error "Let's Encrypt issued the certificate, but ProxyCTL could not sync the managed copy. Certbot lineage remains under $(certbot_config_dir)."
        return 1
    fi
...
# Compatibility public API retained from the original ProxyCTL stub.
# Usage: cert_acme_issue SUBJECT EMAIL [dns-cloudflare|http|dns-manual] [force]
cert_acme_issue() {
    local subject="${1:-}" email="${2:-}" method="${3:-http}" force="${4:-0}"
    with_lock cert _cert_acme_issue_locked "$subject" "$email" "$method" "$force"
}
...
_cert_generate_self_locked() {
    ...
    if ! _cert_replace_pair "$identifier" "$cert" "$key" changed; then
        rm -rf -- "$tmpdir"
        return 1
    fi
    rm -rf -- "$tmpdir"
...
cert_generate_self() {
    local subject="${1:-}"
    with_lock cert _cert_generate_self_locked "$subject"
}
...
cert_import() {
    with_lock cert _cert_import_locked "${1:-}" "${2:-}" "${3:-}"
}
...
cert_info() {
    ...
    printf 'Identifier: %s\n' "$identifier"
    printf 'Subject: %s\n' "$subject"
    printf 'Certbot name: %s\n' "$cert_name"
    printf 'Source: %s\n' "$source"
    printf 'Validation: %s\n' "$validation"
    printf 'Auto renew: %s\n' "$auto_renew"
    printf 'Updated: %s\n' "$updated"
    printf 'Full chain: %s\n' "$(cert_fullchain "$identifier")"
    printf 'Private key: %s\n' "$(cert_privkey "$identifier")"
...
cert_count() {
    metadata_cert_list | grep -c . 2>/dev/null || printf '%s\n' '0'
}

_cert_renew_one_locked() {
    local identifier="$1" __result_var="${2:-}"
    local cert_name validation source owner service before_serial='' after_serial=''
    local result=failed sync_changed=0 lineage_changed=0
    local live_cert
    ...
    case "$validation" in
        dns-cloudflare)
            if ! cert_cloudflare_credentials_available; then
        ...
        http-standalone)
            ...
            case "$owner" in
                ...
                xray|singbox)
                    if [[ "$(system_init 2>/dev/null || true)" != systemd ]]; then
                        warn "${identifier}: port 80 is owned by ${owner}; automatic pre/post hooks require systemd."
                        [[ -n "$__result_var" ]] && printf -v "$__result_var" '%s' blocked
                        return 0
                    fi
                    ;;
    ...
    cert_ensure_certbot_environment || return 1
    local args=(renew --cert-name "$cert_name" --quiet)
    ...
    if (( lineage_changed == 1 )); then result=renewed; else result=unchanged; fi
...
cert_renew() {
    with_lock cert _cert_renew_one_command_locked "${1:-}"
}
...
_cert_renew_all_locked() {
    ...
    while IFS= read -r id; do
        [[ -n "$id" ]] || continue
        result=''
        if ! _cert_renew_one_locked "$id" result; then
...
cert_renew_all() {
    with_lock cert _cert_renew_all_locked
}
...
_cert_delete_locked() {
    ...
    if [[ -n "$consumers" ]]; then
        error "Certificate ${identifier} is still referenced by: $(printf '%s' "$consumers" | paste -sd ',')"
        return 1
    fi
...
cert_delete() {
    with_lock cert _cert_delete_locked "${1:-}"
}
