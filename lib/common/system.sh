...
# Maps uname -m output to a canonical ProxyCTL architecture token.
# Exposed as a separate function so tests can cover the full mapping table
# without mocking uname.
_system_arch_from_machine() {
    local machine="$1"
    case "$machine" in
...
# --- system_distro -----------------------------------------------------------
# Normalised, supported distribution name.
#   debian ubuntu alpine centos rocky almalinux fedora arch
# RHEL-family IDs (rhel, ol, amzn) are reported as "rhel".
system_distro() {
    local id like
    id=$(system_distro_id) || {
        error 'Unable to determine Linux distribution (missing /etc/os-release)'
        return 1
    }
...
# --- system_version ----------------------------------------------------------
# Distro version from VERSION_ID (e.g. 12, 24.04).
system_version() {
    system_os_release_value 'VERSION_ID'
}

# --- system_init -------------------------------------------------------------
# Detects the init system. Returns systemd / openrc / unsupported.
# PROXYCTL_TEST_INIT forces a value so service tests are deterministic.
system_init() {
    if [[ -n "${PROXYCTL_TEST_INIT:-}" ]]; then
        case "$PROXYCTL_TEST_INIT" in
            systemd|openrc) printf '%s\n' "$PROXYCTL_TEST_INIT"; return 0 ;;
...
system_package_manager() {
    ...
    # Fallback: detect the installed package manager directly.
