...
engine_singbox_inbound_list() {
    ...
    jq -r '.inbounds[]? | [.tag,.type,(.listen // "0.0.0.0"),(.listen_port|tostring),(.transport.type // "-"),(if .tls.reality.enabled==true then "reality" elif .tls.enabled==true then "tls" else "none" end),(((.users // [])|length)|tostring)] | @tsv' "$config" | awk -F'\t' 'BEGIN{printf "%-24s %-10s %-18s %-7s %-10s %-9s %s\n","TAG","PROTO","LISTEN","PORT","TRANSPORT","SECURITY","USERS"} {printf "%-24s %-10s %-18s %-7s %-10s %-9s %s\n",$1,$2,$3,$4,$5,$6,$7}'
}
...
engine_singbox_inbound_share() {
    ...
    case "$type" in
        vless|trojan)
            ...
            while IFS=$'\t' read -r name credential; do [[ -z "$wanted" || "$wanted" == "$name" ]] || continue; flow=''; [[ "$type" != vless ]] || flow=$(jq -r --arg tag "$tag" --arg name "$name" '.inbounds[]|select(.tag==$tag)|.users[]|select(.name==$name)|.flow // empty' "$config"); if [[ "$type" == vless ]]; then printf 'vless://%s@%s:%s?%s%s#%s\n' "$credential" "$uri_host" "$port" "$query" "$([[ -n "$flow" ]] && printf '&flow=%s' "$(_singbox_uri_encode "$flow")")" "$(_singbox_uri_encode "${tag}-${name}")"; else printf 'trojan://%s@%s:%s?%s#%s\n' "$(_singbox_uri_encode "$credential")" "$uri_host" "$port" "$query" "$(_singbox_uri_encode "${tag}-${name}")"; fi; done < <(engine_singbox_inbound_clients "$tag")
            ;;
        hysteria2)
            ...
            while IFS=$'\t' read -r name credential; do [[ -z "$wanted" || "$wanted" == "$name" ]] || continue; query="sni=$(_singbox_uri_encode "$sni")"; [[ -z "$obfs" ]] || query+="&obfs=$(_singbox_uri_encode "$obfs")&obfs-password=$(_singbox_uri_encode "$obfs_password")"; printf 'hysteria2://%s@%s:%s?%s#%s\n' "$(_singbox_uri_encode "$credential")" "$uri_host" "$share_port" "$query" "$(_singbox_uri_encode "${tag}-${name}")"; done < <(engine_singbox_inbound_clients "$tag")
            ;;
        anytls)
            while IFS=$'\t' read -r name credential; do [[ -z "$wanted" || "$wanted" == "$name" ]] || continue; if [[ "$security" == reality ]]; then json=$(jq -cn --arg server "$host" --argjson port "$port" --arg password "$credential" --arg sni "$sni" --arg public "$public" --arg sid "$sid" '{type:"anytls",server:$server,server_port:$port,password:$password,tls:{enabled:true,server_name:$sni,reality:{enabled:true,public_key:$public,short_id:$sid}}}'); printf '%s\n' "$json"; else printf 'anytls://%s@%s:%s/?sni=%s#%s\n' "$(_singbox_uri_encode "$credential")" "$uri_host" "$port" "$(_singbox_uri_encode "$sni")" "$(_singbox_uri_encode "${tag}-${name}")"; fi; done < <(engine_singbox_inbound_clients "$tag")
            ;;
        socks|http)
            if [[ $(jq --arg tag "$tag" '.inbounds[]|select(.tag==$tag)|.users|length' "$config") == 0 ]]; then printf '%s://%s:%s\n' "$type" "$uri_host" "$port"; else while IFS=$'\t' read -r name credential; do [[ -z "$wanted" || "$wanted" == "$name" ]] || continue; printf '%s://%s:%s@%s:%s#%s\n' "$type" "$(_singbox_uri_encode "$name")" "$(_singbox_uri_encode "$credential")" "$uri_host" "$port" "$(_singbox_uri_encode "${tag}-${name}")"; done < <(engine_singbox_inbound_clients "$tag"); fi
            ;;
    esac
}
