...
_engine_singbox_install_impl() {
    local version="${1:-}" installer config had_config=0 pm
    ...
    if [[ "$pm" == apk ]]; then
        if [[ -n "$version" ]]; then
            warn 'Exact sing-box versions are not pinned through apk; installing the repository package.'
        fi
    ...
    if (( had_config == 0 )); then _engine_singbox_write_default_config || return 1; fi
...
engine_singbox_version() { if engine_singbox_installed; then sing-box version 2>&1 | head -1 || echo 'unknown'; else echo 'not installed'; fi; }
